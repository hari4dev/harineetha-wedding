...
function Hero() { return <section className="hero section"><Petals /><motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: .8 }}><img src="/images/gallery/ganesha.png" alt="Ganesha" className="heroGanesha" /><div className="crest">KALYANAM</div><h1>{couple.groom}<span>♥</span>{couple.bride}</h1><h2>{couple.headline}</h2><p>{couple.date}</p><div className="scrollHint">Scroll to begin ↓</div></motion.div></section> }
...
function Countdown() {
  ...
  <img className="countdownBg" src="images/designs/kolam.svg" alt="" />

  <div className="topDecor leftDecor">
    <img src="images/designs/jasmine-garland.svg" alt="" />
    <img className="diyaImg" src="images/designs/brass-diya.svg" alt="" />
  </div>
  ...
  <motion.div className="countdownContent">
    ...
    <div className="lotusDivider">
      <span></span>
      <img src="images/designs/lotus.svg" alt="" />
      <span></span>
    </div>
...
function RSVP() { const [form, setForm] = useState({ name: '', email: '', phone: '', groupSize: 1, rsvps: {} }); const max = Number(form.groupSize) || 1; const update = (id, obj) => setForm(f => ({ ...f, rsvps: { ...f.rsvps, [id]: { ...(f.rsvps[id] || { attending: false, count: 0 }), ...obj } } })); const submit = (e) => { e.preventDefault(); const any = Object.values(form.rsvps).some(r => r.attending); if (!any) return alert('Please select at least one event.'); confetti(); alert('RSVP received! Confirmation email + calendar invite can be connected in backend.'); console.log('RSVP Payload:', form); }; return <section className="section rsvp"><h2>RSVP</h2><form onSubmit={submit} className="rsvpCard"><input required placeholder="Full Name" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /><input required type="email" placeholder="Email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} /><input required placeholder="Phone Number" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} /><label>Total Group Size<select value={form.groupSize} onChange={e => setForm({ ...form, groupSize: e.target.value })}>{Array.from({ length: 15 }, (_, i) => i + 1).map(n => <option key={n}>{n}</option>)}</select></label>{events.map(ev => { const r = form.rsvps[ev.id] || { attending: false, count: 0 }; return <div className="rsvpEvent" key={ev.id}><h3>{ev.icon} {ev.title}</h3><div className="toggle"><button type="button" className={r.attending ? 'active' : ''} onClick={() => update(ev.id, { attending: true, count: r.count || 1 })}>Attending</button><button type="button" className={!r.attending ? 'active muted' : ''} onClick={() => update(ev.id, { attending: false, count: 0 })}>Not Attending</button></div>{r.attending && <select value={r.count} onChange={e => update(ev.id, { count: Number(e.target.value) })}>{Array.from({ length: max }, (_, i) => i + 1).map(n => <option key={n}>{n} Guest{n > 1 ? 's' : ''}</option>)}</select>}</div> })}<button className="submit">Submit RSVP</button></form></section> }
